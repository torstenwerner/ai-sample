...
class YouTubeSummarizer:
    ...
    def fetch_transcript(self):
        self.open_database()
        self.cursor.execute("SELECT t.id, t.language, t.transcript FROM transcript t where t.id = ?", (video_id,))
        ...
        if row is not None:
            ...
            if transcript is not None:
                ...
                self.cursor.execute("INSERT INTO transcript VALUES (?, ?, ?)", (video_id, self.language, full_text))
                self.conn.commit()
            else:
